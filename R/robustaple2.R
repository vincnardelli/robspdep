#' Permutation test for Robust APLE statistic 2
#'
#'
...
#'
#' @examples
#' robustaple2(sampledata$x,sampledata$w)


robustaple2 <- function(x, listw, alternative = "greater", nsim=999){
  ...
  robustaple_boot <- function(x, i, x_lag, listw, nsim=nsim, ...) {
    ...
    den <- sum(abs(x_lag))+trW2*t(z)%*%z/n
    return(num/den/2)
  }
  ...
  ret <- list(statistic = statistic,
              p.value = pval,
              res = res)
  class(ret) <- "robspdep.robustaple2"
