#' Permutation test for GK statistic 2
#'
#'
...
#'
#' @examples
#' gk2(sampledata$x,sampledata$w)


gk2 <- function(x, listw, alternative = "greater", nsim=999){

  gk2_boot <- function(x, i, x_lag, listw, nsim=nsim, ...) {
    x <- x[i]
    a<-1/mad(x)
  ...
  res3 <- boot::boot(x, statistic = gk2_boot,
               R = nsim,
               sim = "permutation",
               listw = listw,
               x_lag=x_lag)
